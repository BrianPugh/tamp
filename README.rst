...
Speed optimizations work out to about 2.5-4x faster compression on ESP32s (both Xtensa and RISC-V, i.e. ESP32, 
ESP32-C2, ESP32-S3 &c.).
When built for an **ESP32-S3**, the MCU's SIMD instructions ("PIE") are used which improves compression speed/
...
speed-optimized variant in menuconfig.

Note: The optimized variant leverages the ESP32s' ability to do unaligned 16- and 32-bit memory accesses.
While it should be able to run on architectures other than the ESP32s, it will *not* work on chips which
don't support unaligned memory accesses (e.g. the Raspberry Pi Pico RP2040).
