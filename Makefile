...
# STM32H7B0 On-Device
######################
# Bare-metal benchmark in devices/stm32h7b0/, flashed and run over an ST-Link
# with OpenOCD (semihosting console, no serial port needed). The 128K internal
# flash cannot embed the benchmark data, so run.tcl loads a packed blob into
# AXI SRAM before resuming.
.PHONY: stm32h7b0-device-build stm32h7b0-device-test stm32h7b0-device-benchmark
